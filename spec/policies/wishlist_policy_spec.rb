...
describe WishlistPolicy do
  ...
  permissions :index?, :update?, :destroy? do
    it "grants access to owner" do
      ...
      expect(subject).to permit(seller_context, wishlist)
    end

    it "denies access to accountant" do
      seller_context = SellerContext.new(user: accountant_for_seller, seller:)
      expect(subject).to_not permit(seller_context, wishlist)
    end
  ...
  permissions :update?, :destroy? do
    let(:wishlist) { create(:wishlist, user: create(:user)) }

    it "denies access to another user's wishlist" do
      seller_context = SellerContext.new(user: seller, seller:)
      expect(subject).to_not permit(seller_context, wishlist)
    end
  end
