...
describe AffiliateRequestPolicy do
  ...
  permissions :update?, :approve_all? do
    it "accountant access to support" do
      seller_context = SellerContext.new(user: support_for_seller, seller:)
      expect(subject).not_to permit(seller_context, AffiliateRequest)
    end

    it "grants access to owner" do
      seller_context = SellerContext.new(user: seller, seller:)
      expect(subject).to permit(seller_context, AffiliateRequest)
    end
