...
describe ThumbnailPolicy do
  ...
  permissions :create?, :destroy? do
    it "grants access to owner" do
      ...
      expect(subject).to permit(seller_context, Thumbnail)
    end

    it "denies access to accountant" do
      seller_context = SellerContext.new(user: accountant_for_seller, seller:)
      expect(subject).to_not permit(seller_context, Thumbnail)
    end
    ...
    it "denies access to support" do
      ...
      expect(subject).not_to permit(seller_context, Thumbnail)
    end
  end
end
