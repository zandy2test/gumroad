...
describe DropboxFilesPolicy do
  ...
  permissions :create?, :index?, :cancel_upload? do
    it "grants access to owner" do
      seller_context = SellerContext.new(user: seller, seller:)
      expect(subject).to permit(seller_context, :dropbox_files)
    end

    it "denies access to accountant" do
      seller_context = SellerContext.new(user: accountant_for_seller, seller:)
      expect(subject).to_not permit(seller_context, :dropbox_files)
    end
    ...
    it "denies access to support" do
      ...
      expect(subject).not_to permit(seller_context, :dropbox_files)
    end
  end
end
