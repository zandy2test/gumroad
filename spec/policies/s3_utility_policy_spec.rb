...
describe S3UtilityPolicy do
  ...
  permissions :generate_multipart_signature?, :current_utc_time_string?, :cdn_url_for_blob? do
    it "grants access to owner" do
      ...
      expect(subject).to permit(seller_context, S3UtilityPolicy)
    end

    it "denies access to accountant" do
      seller_context = SellerContext.new(user: accountant_for_seller, seller:)
      expect(subject).to_not permit(seller_context, S3UtilityPolicy)
    end
    ...
    it "denies access to support" do
      ...
      expect(subject).not_to permit(seller_context, S3UtilityPolicy)
    end
  end
end
